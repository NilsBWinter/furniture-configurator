	...
	<div>
		...
		<div>
			...
			<div v-for="(box, index) in userBoxes" :key="index" :value="box" class="box-container">
				...
				<div v-html="makerjs.exporter.toSVG(boxCoordinates(box, materialRef).groundSide)"></div>
			</div>
...
import { downloadSVG } from '../store/svg';
...
export default {
	...
	setup(props) {
		...
		const testUserBoxes = reactive<Array<Box>>([
			reactive(
				createTestBox(
					shelf,
					possibleUserBoxDimesnions.value,
					1,
					1,
					possibleUserBoxDimesnions.value.heights[0],
					possibleUserBoxDimesnions.value.widths[1],
				),
			),
			reactive(
				createTestBox(
			...
			reactive(
				createTestBox(
					...
					2,
					2,
					possibleUserBoxDimesnions.value.heights[1],
					possibleUserBoxDimesnions.value.widths[1],
				),
			),
			reactive(
				createTestBox(
					...
					5,
					1,
					possibleUserBoxDimesnions.value.heights[2],
					possibleUserBoxDimesnions.value.widths[0],
				),
			),
			reactive(
				createTestBox(
					...
					4,
					possibleUserBoxDimesnions.value.heights[0],
					possibleUserBoxDimesnions.value.widths[3],
				),
			),
		]);
